...
from .modules.sba_module import SBA
from .modules.mfm_module import MFM
from network.modules.dysample_module import DySample
...
class Regression(nn.Module):
    def __init__(self, use_sba: bool = False, use_mfm: bool = False):
        ...
        # Using DySample for upscaling feature maps
        self.dysample0 = DySample(128, scale=2, style='lp')
        self.dysample1 = DySample(256, scale=2, style='lp')
        ...
        # optional fusion modules (SBA / MFM) are expected to be defined elsewhere
        if self.use_sba:
            # edge-aware gating fusion
            self.sba2 = SBA(input_dim=512, output_dim=512)
            self.sba1 = SBA(input_dim=256, output_dim=256)
            self.sba0 = SBA(input_dim=128, output_dim=128)

        if self.use_mfm:
            # modulation fusion module (two-input feature fusion)
            self.mfm2 = MFM(dim=512)
            self.mfm1 = MFM(dim=256)
            self.mfm0 = MFM(dim=128)
    ...
    def forward(self, x, cls):
        ...
        # multi-scale fusion: baseline(add), MFM, SBA, or MFM+SBA
        if self.use_sba and self.use_mfm:
            # 前两层用 MFM，最高分辨率用 SBA（H:来自浅层，L:来自上一层融合）
            x2_1 = self.mfm2(self.ca2(x2), self.v3(x3))
            x1_1 = self.mfm1(self.ca1(x1), self.v2(x2_1))

            h0 = self.ca0(x0)
            l0 = self.v1(x1_1)
            x0_1 = self.sba0(h0, l0)

        elif self.use_sba:
            # 三层都用 SBA
            h2 = self.ca2(x2)
            l2 = self.v3(x3)
            x2_1 = self.sba2(h2, l2)

            h1 = self.ca1(x1)
            l1 = self.v2(x2_1)
            x1_1 = self.sba1(h1, l1)
            ...
            x2_1 = self.mfm2(self.ca2(x2), self.v3(x3))
            x1_1 = self.mfm1(self.ca1(x1), self.v2(x2_1))
            x0_1 = self.mfm0(self.ca0(x0), self.v1(x1_1))
        ...
        if self.training:
            ...
            y1 = self.res1(self.dysample1(self.noise1(x1_1)))
            ...
            y1 = self.res1(self.dysample1(x1_1))
            y0 = self.res0(x0_1)
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 use_adapter: bool = False, use_sba: bool = False, use_mfm: bool = False):
        super().__init__()
        self.num_classes = num_classes
        self.depths = depths
        self.num_stages = num_stages

        # decoder / fusion configs
        ...
        # regression (density decoder) with configurable fusion
        self.regression = Regression(use_sba=self.use_sba, use_mfm=self.use_mfm)

        # optional feature adapter between backbone and decoder
        if self.use_adapter:
            self.adapter = PVTAdapter()
    ...
    def forward(self, label_x, unlabel_x=None):
        ...
        if self.training:
            ...
            if self.use_adapter:
                unlabel_x = self.adapter(unlabel_x)
            ...
            return [label_x_1, label_x_2, label_x_3], [unlabel_x_1, unlabel_x_2,
                                                       unlabel_x_3], label_normed, out_cls_l, out_cls_ul
...
# Plain alias to avoid @register_model naming conflicts
def pvt_treeformer_dy(pretrained=False, **kwargs):
    return pvt_treeformer(pretrained=pretrained, **kwargs)